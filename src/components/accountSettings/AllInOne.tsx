...
export default function AllInOne(props: { setOption:any}) {
  ...
  const updateContactHealthtInfo = async () => {
    ...
    if (formData.email !== '') await userDoc.update({ email: formData.email });
    if (formData.dateOfBirth !== null) await userDoc.update({ dateOfBirth: formData.dateOfBirth });
